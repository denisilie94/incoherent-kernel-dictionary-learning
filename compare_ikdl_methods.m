...
for r = 1:n_rounds
    ...
    A0_all = cell(1, n_classes);
    for i_class = 1:n_classes
        A0_all{i_class} = normcol_equal(randn(n_signals / n_classes, n_components));
    end
    ...
    for row = 1:n_signals
       for col = row:n_signals
           x = X_train(:,row);
           y = X_train(:,col);
           K_all(row, col) = kernel_function(x, y, sigma);
           K_all(col, row) = K_all(row, col);
       end
    end
    kernelTrainTime = toc(tmpTime);
    trainTime = trainTime + kernelTrainTime;

    for i_iter = 1:n_iterations
        tmpTime = tic;
        for i_class = 1:n_classes
            % Get current dictionary and kernel
            A = A_all{i_class};
    ...
    kernelTestTime = toc(tmpTime);
    testTime = testTime + kernelTestTime;
    ...
    AccuracyA = AccuracyA + accuracy;
    TrainTimeA = TrainTimeA + trainTime;
    TestTimeA = TestTimeA + testTime;
    ...
    % Start waitbar
    trainTime = kernelTrainTime;
    wb = waitbar(0, '[UAK-SVD] Training...');
    ompparams = {'checkdict', 'off'};
    ...
    for i_iter = 1:n_iterations
        ...
        for i_class = 1:n_classes
            ...
            for i_atom = 1:size(A, 2)
                ...
                if (isempty(atom_usages))
                    ...
                    a = K * F * X(i_atom, atom_usages)' - 2 * gamma * (Kc * Kc') * A(:, i_atom);
                    A(:, i_atom) = a / sqrt(a' * K * a);
                    X(i_atom, atom_usages) = (E(:, atom_usages) + A(:, i_atom) * X(i_atom, atom_usages))' * K * A(:, i_atom);
                    E(:, atom_usages) = F - A(:, i_atom) * X(i_atom, atom_usages);
                end
            end
        ...
        % Update waitbar
        waitbar(i_iter/n_iterations, wb, sprintf('[UAK-SVD] Training - Remaining time: %d [sec]',...
                round(trainTime/i_iter*(n_iterations - i_iter))));
    end
    ...
    testTime = kernelTestTime;
    wb = waitbar(0, '[UAK-SVD] Testing...');
    ...
    for i_test = 1:size(X_test, 2)
       ...
       % Update waitbar
       waitbar(i_test/size(X_test, 2), wb, sprintf('[UAK-SVD] Testing - Remaining time: %d [sec]',...
               round(testTime/i_test*(size(X_test, 2) - i_test))));
    end
    ...
    % Compute problem accuracy
    accuracy = sum(y_test==prediction)/size(X_test,2);
    AccuracyN = AccuracyN + accuracy;
    TrainTimeN = TrainTimeN + trainTime;
    TestTimeN = TestTimeN + testTime;

    fprintf("[UAK-SVD] Accuracy: %f\n", accuracy);
    fprintf('[UAK-SVD] Training time: %f [sec]\n', trainTime);
    fprintf('[UAK-SVD] Testing time: %f [sec]\n', testTime);
    fprintf('\n');
end
