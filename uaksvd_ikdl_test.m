...
% start waitbar
trainTime = 0;
wb = waitbar(0, '[Kernel UAK-SVD] Training...');
...
for i_iter = 1:n_iterations
    ...
    % update waitbar
    waitbar(i_iter/n_iterations, wb, sprintf('[Kernel UAK-SVD] Training - Remaining time: %d [sec]',...
...
testTime = 0;
wb = waitbar(0, '[Kernel UAK-SVD] Testing...');
...
for i_test = 1:size(X_test, 2)
   ...
   % update waitbar
   waitbar(i_test/size(X_test, 2), wb, sprintf('[Kernel UAK-SVD] Testing - Remaining time: %d [sec]',...
...
fprintf("[Kernel UAK-SVD] Accuracy: %f\n", accuracy);
fprintf('[Kernel UAK-SVD] Training time: %f [sec]\n', trainTime);
fprintf('[Kernel UAK-SVD] Testing time: %f [sec]\n', testTime);
fprintf('\n');
