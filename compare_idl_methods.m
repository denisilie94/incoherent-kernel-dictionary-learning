...
for r = 1:n_rounds
    ...
    D0_all = cell(1, n_classes);
    for i_class = 1:n_classes
        D0_all{i_class} = normcol_equal(randn(size(X_train,1), n_components));
    end
        ...
        for i_class = 1:n_classes
            ...
            for i_atom = 1:size(D, 2)
                ...
                if (isempty(atom_usages))
                    ...
                    d = F * X(i_atom, atom_usages)' - 2 * gamma * (Dc * Dc') * D(:, i_atom);
                    D(:, i_atom) = d / norm(d);
                    X(i_atom, atom_usages) = (E(:, atom_usages)' * D(:, i_atom))' + X(i_atom, atom_usages);
                    E(:, atom_usages) = F - D(:, i_atom) * X(i_atom, atom_usages);
                end
            end
